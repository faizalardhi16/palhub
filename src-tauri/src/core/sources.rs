...
/// Fetch a skill from `github:owner/repo[#subdir]`, `npm:package[@version]`,
/// or `local:/path/to/skill` (a folder containing SKILL.md).
///
...
pub fn fetch(source: &str) -> Result<(String, PathBuf)> {
    ...
    if let Some(rest) = source.strip_prefix("github:") {
        ...
        bail!("unsupported source (use github:owner/repo, npm:package, or local:/path): {source}")
    }
}

/// Copy a local skill folder (`local:/abs/path/to/skill`) into a temp dir so the
/// store can rename it into place. Useful for knowledge bundles generated by
/// harvesters (e.g. pal-knowledge) without a git push in between.
fn fetch_local(path_spec: &str) -> Result<(String, PathBuf)> {
    let src = PathBuf::from(path_spec);
    if !src.join("SKILL.md").exists() {
        bail!("local source has no SKILL.md: {}", src.display());
    }
    let name = src
        .file_name()
        .map(|n| n.to_string_lossy().to_string())
        .ok_or_else(|| anyhow!("cannot derive skill name from path"))?;
    if name.is_empty() || name.contains(['/', '\\', ' ', '.']) {
        bail!("invalid skill name derived from path: {name:?} (use a folder name like finance-id)");
    }
